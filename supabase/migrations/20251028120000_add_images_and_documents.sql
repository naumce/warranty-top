-- Enhanced Warranty Tracker Migration
-- Adds support for: receipt photos, product images, documents, store locations, OCR data

-- Add new columns to warranties table
ALTER TABLE public.warranties
ADD COLUMN IF NOT EXISTS receipt_number TEXT,
ADD COLUMN IF NOT EXISTS order_number TEXT,
ADD COLUMN IF NOT EXISTS purchase_method TEXT, -- 'cash', 'card', 'online', etc.
...
ADD COLUMN IF NOT EXISTS warranty_type TEXT, -- 'manufacturer', 'extended', 'store', etc.
ADD COLUMN IF NOT EXISTS condition_at_purchase TEXT, -- 'new', 'used', 'refurbished'
ADD COLUMN IF NOT EXISTS reminder_enabled BOOLEAN DEFAULT true,
ADD COLUMN IF NOT EXISTS reminder_days_before INTEGER[] DEFAULT ARRAY[30, 14, 7], -- Days before expiry to send reminders
ADD COLUMN IF NOT EXISTS last_reminder_sent_at TIMESTAMP WITH TIME ZONE,
ADD COLUMN IF NOT EXISTS tags TEXT[], -- Custom tags for organization
ADD COLUMN IF NOT EXISTS receipt_ocr_data JSONB; -- Raw OCR data from receipt
...
CREATE TABLE IF NOT EXISTS public.warranty_images (
  ...
  image_type TEXT NOT NULL, -- 'product', 'receipt', 'damage', 'packaging', 'other'
  caption TEXT,
...
CREATE TABLE IF NOT EXISTS public.warranty_documents (
  ...
  storage_path TEXT NOT NULL,
  document_type TEXT NOT NULL, -- 'warranty_certificate', 'manual', 'receipt', 'invoice', 'extended_warranty', 'insurance', 'other'
...
CREATE POLICY "System can create reminders"
  ON public.warranty_reminders
  FOR INSERT
  WITH CHECK (true); -- System/service can create reminders
...
DECLARE
  v_warranty_id UUID;
BEGIN
  -- Get warranty_id from the image
  SELECT warranty_id INTO v_warranty_id
  FROM public.warranty_images
  WHERE id = p_image_id;

  -- Unset all other images as primary for this warranty
